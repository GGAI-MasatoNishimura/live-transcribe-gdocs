...
| ディレクトリ | 役割 |
|--------------|------|
| [extension/](extension/) | Chrome 拡張（MV3）。Meet タブ ID の記録、タブ音声とマイクのミックス、録音中は Meet タブ音声のみスピーカーモニター、ローカル relay への WebSocket。 |
...
7. **Meet とマイクを許可して録音開始** を押し、ブラウザの許可に従う。**録音中**は Meet の声だけがスピーカーにも流れます（マイクの声はスピーカーに返しません。ハウリングしやすい場合はヘッドセット推奨）。
